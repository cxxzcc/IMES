...
public class JdbcConfig {
    ...
    public void insertSystemLog(IapOpsLogTDto iapOpsLogDto) throws Exception {
        ...
        PreparedStatement ps = null;
        try {
            String sql = "INSERT INTO iap_ops_log_t ( id, service_id, service_name, service_ip, namespace, method_type, request_function, request_method, request_proxy, request_params, log_data, create_date, last_update_date, method_desc, EXECUTION_STATUS, EXECUTION_FLAG, INTERACTIVE_SYSTEM) " +
                    "VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ) ";
            Timestamp date = this.getSqlDate();
            ps = connection.prepareStatement(sql);
            ...
            ps.setString(15, iapOpsLogDto.getExecutionStatus());
            ps.setString(16, iapOpsLogDto.getExecutionFlag());
            ps.setString(17, iapOpsLogDto.getInteractiveSystem());
            ps.addBatch();
            ps.executeBatch();
            connection.commit();
        } catch (SQLException throwables) {
