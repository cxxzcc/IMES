...
public interface ScheduleMapper extends BaseMapper<ScheduleEntity> {
    ...
    IPage<ScheduleRespTwoDTO> findTwoList(Page page, @Param("schedule") ScheduleQueryTwoDTO scheduleQueryTwoDTO);

    List<ScheduleRespTwoDTO> findTwoPage(Page<ScheduleRespTwoDTO> page, @Param("params") Map<String, Object> params);

    ScheduleDetailRespDTO findById(String bo);

    ScheduleSaveTwoDTO findTwoById(String schedulePlineBo);

    ScheduleRespTwoDTO queryTwoById(String bo);

    List<ScheduleDetailRespDTO> findByIdWithCount(@Param("bo") String bo, @Param("productLineBo") String productLineBo);
    /**
     * 根据车间BO查询产线
     * @param workShopBo
     * @return
