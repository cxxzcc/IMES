...
public class CapacityLoadServiceImpl implements CapacityLoadService {
    ...
    public CapacityLoadReportDTO findCapacityLoadList(CapacityLoadQueryDTO capacityLoadQueryDTO) {
        ...
        //取机台产能额定开机时间和机台信息
        List<CapacityLoadRespDTO> machineAndRatedTimes = capacityLoadMapper.machineRatedTime(UserUtils.getSite(), capacityLoadQueryDTO);

        //取时间段的负荷报表信息
        List<CapacityLoadRespDTO> capacityLoadRespDTOList = capacityLoadMapper.capacityLoadList(UserUtils.getSite(),startDate,endDate);
