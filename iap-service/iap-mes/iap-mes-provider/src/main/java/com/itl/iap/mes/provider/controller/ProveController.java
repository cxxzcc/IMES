...
public class ProveController {
    ...
    @ApiOperation(value = "feign接口:根据id列表查询")
    public ResponseData<List<ProveEntity>> getByIds(@RequestBody List<String> ids) {
        return ResponseData.success(proveService.getByIds(ids));
    }

    @GetMapping("/userId/{userId}")
    @ApiOperation(value = "feign接口:根据id列表查询")
    public ResponseData<List<ProveEntity>> getByUserId(@PathVariable("userId") String userId, @RequestParam("site") String site) {
        return ResponseData.success(proveService.getByUserId(userId, site));
    }
