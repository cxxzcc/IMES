...
public class FaultController extends BaseController {
    ...
    public ResponseData importFile(@RequestParam("file") MultipartFile file) throws CommonException {
    ...
    public String download(HttpServletResponse response) {
        ...
        try {
            ...
            response.setHeader("Content-Disposition", "attachment;filename=异常代码导入模板.xls");
    ...
    @ApiOperation("获取所有启用的故障编码")
    public ResponseData<List<Fault>> getFaults() {
        List<Fault> ret = faultService.lambdaQuery().eq(Fault::getSiteId, UserUtils.getSite()).eq(Fault::getState, 0).list();
