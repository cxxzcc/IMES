...
public class ProveServiceImpl implements ProveService {
    ...
    public IPage<ProveEntity> findList(ProveQueryDTO proveQueryDTO) {
        ...
        QueryWrapper<ProveEntity> queryWrapper = new QueryWrapper<ProveEntity>();
        if (StrUtil.isNotEmpty(proveQueryDTO.getProveCode())) {
            queryWrapper.like("prove_code", proveQueryDTO.getProveCode());
        }
        if (StrUtil.isNotEmpty(proveQueryDTO.getProveDesc())) {
            queryWrapper.like("prove_desc", proveQueryDTO.getProveDesc());
        }
        if (StringUtils.isEmpty(proveQueryDTO.getSite())) {
    ...
    @Override
    public List<ProveEntity> getByIds(List<String> ids) {
        if(CollUtil.isEmpty(ids)) {
            return Collections.emptyList();
        }
        return proveMapper.selectBatchIds(ids);
    }

    @Override
    public List<ProveEntity> getByUserId(String userId, String site) {
        return proveMapper.getByUserId(userId, site);
    }
}
