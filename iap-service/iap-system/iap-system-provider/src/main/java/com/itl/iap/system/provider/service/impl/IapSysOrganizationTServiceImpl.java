...
public class IapSysOrganizationTServiceImpl extends ServiceImpl<IapSysOrganizationTMapper, IapSysOrganizationT> implements IapSysOrganizationTService {
    ...
    @Resource
    private IapPositionTService iapPositionTService;
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Boolean deleteById(IapSysOrganizationTDto organizationDTO) throws CommonException {
        final List<IapPositionTDto> listTreeByOrgId = iapPositionTService.getListTreeByOrgId(new IapPositionTDto().setOrgId(organizationDTO.getId()));
        if (CollUtil.isNotEmpty(listTreeByOrgId)){
            throw new CommonException( "组织已被使用，不能删除", CommonExceptionDefinition.BASIC_EXCEPTION );
        }
    ...
    public List<Map<String, String>> getByIds(Set<String> ids) {
        return organizationMapper.getByIds(ids);
    }

    @Override
    public String getIdByCode(String code) {
        return organizationMapper.getIdByCode(code);
    }

    @Override
    public IPage<IapSysOrganizationTDto> getAll(IapSysOrganizationTDto iapSysOrganizationTDto) {
        if(ObjectUtil.isEmpty(iapSysOrganizationTDto.getPage())){
            iapSysOrganizationTDto.setPage(new Page(1, 10));
        }
        return organizationMapper.getAll(iapSysOrganizationTDto.getPage(),iapSysOrganizationTDto);
    }
}
