...
public class IapSysResourceTServiceImpl extends ServiceImpl<IapSysResourceTMapper, IapSysResourceT> implements IapSysResourceTService {
     ...
     */
    @Override
    public List<IapSysResourceTDto> queryList(String clientType) {
        // 获取客户端列表
        List<AuthClientDto> allClient = iapSysResourceMapper.getAllClient(new AuthClientDto());
        // 获取菜单
        List<IapSysResourceTDto> iapSysResourceDtos = iapSysResourceMapper.queryList(clientType);
...
//                return iapSysResourceTDtos;
     ...
     */
    @Override
    public List<IapSysResourceTDto> getAllMenu(String clientType) {
        ...
        List<IapSysResourceTDto> allMenu = iapSysResourceMapper.getAllMenu(user.getUserName(), clientType).stream().sorted(Comparator.comparing(IapSysResourceTDto::getSort)).collect(Collectors.toList());
    ...
    @Override
    public List<IapSysResourceTDto> getAllMenuTwo(String clientType) {
        UserTDto user = userUtil.getUser();
        List<AuthClientDto> allClient = iapSysResourceMapper.getAllClient(new AuthClientDto().setClientId(user.getClientId()));
        List<IapSysResourceTDto> allMenu = iapSysResourceMapper.getAllMenuTwo(user.getUserName(), clientType).stream().sorted(Comparator.comparing(IapSysResourceTDto::getSort)).collect(Collectors.toList());
        List<IapSysResourceTDto> collect = Lists.newArrayList();
        for (AuthClientDto auth : allClient) {
            List<IapSysResourceTDto> lists = allMenu.stream().filter(x -> x.getParentId().equals(auth.getId())).collect(Collectors.toList());
            for (IapSysResourceTDto iapSysResourceDto : lists) {
