...
import com.itl.iap.common.util.PwdCheckUtil;
import com.itl.iap.common.util.UUID;
import com.itl.iap.system.api.dto.ChangePwdDto;
...
public class IapSysUserTController extends BaseController {
    ...
    public ResponseData add(@RequestBody IapSysUserT iapSysUserT) {
        ...
        String userName = iapSysUserT.getUserName();
        List<IapSysUserT> iapSysUserTS = iapSysUserService.queryUserByUserName(userName);
        if (Boolean.FALSE.equals(CollectionUtils.isEmpty(iapSysUserTS))) {
            return ResponseData.error("用户账号已存在");
        }
    ...
    public ResponseData update(@RequestBody IapSysUserT iapSysUserT) {
        ...
        IapSysUserT user = iapSysUserService.getUser(iapSysUserT.getUserName());
        if (user != null && !user.getId().equals(iapSysUserT.getId())) {
            throw new CommonException("账号不能重复", CommonExceptionDefinition.VERIFY_EXCEPTION);
        }
...
//        IapSysUserT user = iapSysUserTService.getOne(new QueryWrapper<IapSysUserT>().lambda().eq(IapSysUserT::getUserName, userName));
    ...
    @GetMapping("/getUser")
    @ApiOperation(value = "根据用户名查询用户信息", notes = "根据用户名查询用户信息")
    public ResponseData<IapSysUserT> getUser(@RequestParam String userName) throws CommonException {
        logger.info("IapSysUserTDto getUser...");
        IapSysUserT userDto = iapSysUserService.getUser(userName);
        return ResponseData.success(userDto);
    }

    @PostMapping("/getUserList")
    @ApiOperation(value = "根据用户名查询用户信息", notes = "根据用户名查询用户信息")
    public ResponseData<List<IapSysUserT>> getUserList(@RequestBody List<String> userNameList) throws CommonException {
        logger.info("IapSysUserTDto getUser...");
        List<IapSysUserT> userDto = iapSysUserService.getUserList(userNameList);
        return ResponseData.success(userDto);
    }
    ...
    public ResponseData<Integer> updateUserPwdById(@RequestBody ChangePwdDto changePwdDto) {
        try {
            return ResponseData.success(iapSysUserService.updateUserPwdById(changePwdDto));
    ...
    @ApiOperation(value = "检测密码强度", notes = "检测密码强度")
    public ResponseData<Boolean> checkPassword(@RequestBody Map<String, String> password) {
        boolean result = true;
        String msg = null;
        try {
            result = PwdCheckUtil.evalPassword(password.get("password"));
        } catch (Exception e) {
            e.printStackTrace();
            msg = e.getMessage();
            result = false;
        }
        return ResponseData.success("200", msg, result);
    }
    ...
    @PostMapping("/userListByIds")
    @ApiOperation(value = "根据id列表查询用户列表", notes = "根据id列表查询用户列表")
    public ResponseData<List<IapSysUserTDto>> getUserListByIds(@RequestBody List<String> ids) {
        if (CollUtil.isEmpty(ids)) {
            return ResponseData.success(Collections.emptyList());
        }
        QueryWrapper<IapSysUserT> wrapper = new QueryWrapper<IapSysUserT>()
                .select("id", "user_name AS userName", "user_mobile AS userMobile", "real_name AS realName", "email", "gender", "birthday", "nick_name AS nickName", "avatar")
                .in("id", ids);
        List<IapSysUserT> list = iapSysUserService.list(wrapper);
        return ResponseData.success(DtoUtils.convertList(IapSysUserTDto.class, list));
    }
