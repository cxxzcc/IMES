...
public class IapSysOrganizationTController {
    ...
    public ResponseData<Boolean> deleteById(@RequestBody IapSysOrganizationTDto organizationDTO) throws CommonException {
    ...
    @ApiOperation(value = "根据id集合获取name集合", notes = "根据id集合获取name集合")
    public List<Map<String, String>> getByIds(@RequestBody Set<String> ids) {
        if (ids.isEmpty()) {
            return null;
        }
        return organizationService.getByIds(ids);
    }

    @GetMapping("/getIdByCode")
    @ApiOperation(value = "根据code获取组织id", notes = "根据code获取组织id")
    public String getIdByCode(@RequestParam String code) {
        return organizationService.getIdByCode(code);
    }

    @PostMapping("/queryLov")
    @ApiOperation(value = "组织LOV", notes = "组织LOV")
    public ResponseData queryList(@RequestBody IapSysOrganizationTDto iapSysOrganizationTDto) {
        return ResponseData.success(organizationService.getAll(iapSysOrganizationTDto));
    }
}
