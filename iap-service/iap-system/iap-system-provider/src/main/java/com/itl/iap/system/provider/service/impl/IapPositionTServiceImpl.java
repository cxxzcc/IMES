...
public class IapPositionTServiceImpl extends ServiceImpl<IapSysPositionTMapper, IapPositionT> implements IapPositionTService {
     ...
     */
    @Override
    public IPage<List<Map<String, Object>>> queryUserListByPositionId(Map<String, Object> map) {
        if (map.get("page") == null) {
            map.put("page", new Page(0, 10));
        }
        Page page = Convert.convert(Page.class, map.get("page"));
        try {
            return page.setRecords(iapSysPositionMapper.queryUserListByPositionId(page, map));
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public Boolean deleteById(IapPositionTDto iapPositionDto) {
        ...
        if (iapPositionDto.getId() != null) {
            ...
            if (childrenNum != 0) {
                ...
                // 删除岗位-组织中间表信息(iap_sys_organization_position)信息
    ...
    public IPage<IapPositionTDto> getListTree(IapPositionTDto iapPositionDto) {
        ...
        List<IapPositionTDto> listTree = iapSysPositionMapper.getListTreeNotUser(iapPositionDto);
...
//            codeTreeList.stream().filter(x -> x.getSort() != null).sorted(Comparator.comparing(IapPositionTDto::getSort)).collect(Collectors.toList());
    ...
    public List<IapPositionTDto> getTree(IapPositionTDto iapPositionDto) {
        ...
        // 先通过前端传入的条件查询
        Set<IapPositionTDto> parentPositionSet = new HashSet<>(iapSysPositionMapper.getListTreeNotUser(iapPositionDto));
        if (parentPositionSet.isEmpty()) {
            return new ArrayList<>(parentPositionSet);
        }
        // 全部岗位列表
        List<IapPositionTDto> allPositionSet = DtoUtils.convertList(IapPositionTDto.class, iapSysPositionMapper.getListTreeNotUser(new IapPositionTDto()));
        // 创建一个子组织set
