...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team> implements TeamService {
    ...
    public TeamVo getTeamVoByTeam(String team) throws CommonException {
        ...
        if(teamEntity.getProductLineBo()!=null){
            ...
            Optional.ofNullable(productLine).ifPresent(x -> teamVo.setProductLine(productLine.getProductLine()));
        }
    ...
    @Override
    public Page<Map<String, Object>> getUsersByTeam(TeamUserQueryDto params) {
        if (params.getPage() == null) {
            params.setPage(new Page(1, 10));
        }
        if (params.getTeam() != null && !"".equals(params.getTeam())) {
            params.setTeamBo(new TeamHandleBO(UserUtils.getSite(), params.getTeam()).getBo());
        }
        return teamMapper.getUsersByTeam(params.getPage(), params);
    }
    ...
    public void export(HttpServletRequest request, HttpServletResponse response) {
        ...
        ExcelUtils.exportExcel(teams, "班组列表", "班组列表", ProductLine.class, "班组列表.xls", true, response);
    }

    @Override
    public void saveByImport(List<TeamVo> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            BeanUtils.copyProperties(teamVo,team);
            team.setBo(teamHandleBO.getBo());
            team.setProductLineBo(new ProductLineHandleBO(UserUtils.getSite(),teamVo.getProductLine()).getBo());
            team.setSite(UserUtils.getSite());
            team.setObjectSetBasicAttribute(userUtil.getUser().getUserName(),new Date());
            teamMapper.insert(team);
        }
