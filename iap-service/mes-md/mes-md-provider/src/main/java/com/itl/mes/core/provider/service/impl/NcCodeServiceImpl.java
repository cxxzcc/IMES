...
public class NcCodeServiceImpl extends ServiceImpl<NcCodeMapper, NcCode> implements NcCodeService {
    ...
    public void saveNcCode(NcCodeVo ncCodeVo) throws CommonException {
        ...
        if(ncCode1Entity== null){
            ...
            ncDispRouterService.saveByNcCodeBo(bo, ncCodeVo.getAssignedNcDispRouterVos());
        if(CollUtil.isNotEmpty(ncCodeVo.getCustomDataValVoList())){
    ...
    @Override
    public NcCode selectByNcCode(String ncCode, String ncName, String ncType, String state) throws CommonException {
        QueryWrapper<NcCode> entityWrapper = new QueryWrapper<>();

        entityWrapper.eq(NcCode.NC_CODE,ncCode);
        entityWrapper.eq(NcCode.SITE,UserUtils.getSite());
        entityWrapper.eq(NcCode.NC_NAME,ncName);
        entityWrapper.eq(NcCode.NC_TYPE,ncType);
        entityWrapper.eq(NcCode.SITE,state);
        List<NcCode> ncCodes = ncCodeMapper.selectList(entityWrapper);
        if(ncCodes.isEmpty()){
            throw new CommonException("不合格代码"+ncCode+"未维护", CommonExceptionDefinition.BASIC_EXCEPTION);
        }else{
    ...
    public NcCodeVo getNcCodeVoByNcCode(String ncCode) throws CommonException {
        ...
        if (!availableNcGroupList.isEmpty()) {
            for (Map map : availableNcGroupList) {
                ...
                groupCodeVo.setNc_GROUP(map.getOrDefault("NC_GROUP","").toString());
                groupCodeVo.setNc_GROUP_NAME(map.getOrDefault("NC_GROUP_NAME","").toString());
                groupCodeVo.setNc_GROUP_DESC(map.getOrDefault("NC_GROUP","").toString());
        ...
        if (!assignedNcGroupList.isEmpty()) {
            for (Map map : assignedNcGroupList) {
                ...
                groupCodeVo.setNc_GROUP(map.getOrDefault("NC_GROUP","").toString());
                groupCodeVo.setNc_GROUP_NAME(map.getOrDefault("NC_GROUP_NAME","").toString());
                groupCodeVo.setNc_GROUP_DESC(map.getOrDefault("NC_GROUP_DESC","").toString());
    ...
    @Override
    public NcCodeVo getNcCodeVoByNcCode(String ncCode, String ncName, String ncType, String state) throws CommonException {
        NcCode ncCodeEntity = selectByNcCode(ncCode , ncName , ncType , state);
        List<CustomDataAndValVo> customDataAndValVos = customDataValService.selectCustomDataAndValByBoAndDataType(UserUtils.getSite(), ncCodeEntity.getBo(), CustomDataTypeEnum.NC_CODE.getDataType());
        List<Map<String, Object>> availableNcGroupList = ncCodeMapper.getAvailableNcGroupList(UserUtils.getSite(), ncCodeEntity.getBo());
        List<GroupCodeVo> available = new ArrayList<GroupCodeVo>();
        if (!availableNcGroupList.isEmpty()) {
    ...
    @Override
    public List<NcCodeVo> getNcCodeByNcgBos(List<String> ncgBos, String site) {
        return baseMapper.getNcCodeByNcgBos(ncgBos, site);
    }

    @Override
    public List<NcCode> getNcCodeByIds(List<String> ncCodeBos, String site) {
        if(CollUtil.isEmpty(ncCodeBos)) {
            return Collections.emptyList();
        }
        return lambdaQuery().eq(NcCode::getSite, site).in(NcCode::getBo, ncCodeBos).list();
    }
}
