 ...
 */
@Service
public class QualitativeInspectionServiceImpl {
    ...
    public List<MeProductInspectionItemsOrderDto> getProductInspectionItems(QualitativeInspectionDTO dto) {
        ...
        return qualitativeInspectionMapper.findProductInspectionItemsByOperationBo(operationBo, new SnHandleBO(site, dto.getSn()).getBo(), site, "0");
    }
     ...
     * @return 是否成功
     * */
    public Boolean saveQualitativeInspection(List<TempDateSaveRequestDto> list, String sn) {
        ...
        for (TempDateSaveRequestDto tempDateSaveRequestDto : list) {
            ...
            //结果，不填默认OK
            item.setResult(StrUtil.isBlank(tempDateSaveRequestDto.getResult()) ? CheckResultEnum.OK.getCode() : tempDateSaveRequestDto.getResult());
        ...
        if(CollUtil.isNotEmpty(qualitativeInspectionSaveDTOS)) {
            ...
            temporaryData.setType(TemporaryDataTypeEnum.QUALITATIVE.getCode());
    ...
    public List<QualitativeInspectionSaveDTO> getBySnAndStation(String sn, String station) {
        TemporaryData temporaryData = temporaryDataService.getBySnAndStation(sn, station, TemporaryDataTypeEnum.QUALITATIVE.getCode());
