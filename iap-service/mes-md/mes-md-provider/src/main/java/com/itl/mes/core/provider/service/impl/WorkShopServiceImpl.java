...
//        WorkShop workShopEntity = workShopMapper.selectById();
        QueryWrapper<WorkShop> wrapper = new QueryWrapper<>();
        wrapper.eq("bo", workShopHandleBO.getBo());
    ...
    public void deleteWorkShop(String workShop, Date modifyDate) throws CommonException {
        ...
        boolean isUsed = validateWorkShopIsUsed(site, workShop);//验证车间是否被使用
        if (isUsed) {
            throw new CommonException("车间已被使用，不能删除", CommonExceptionDefinition.BASIC_EXCEPTION);
        }
        QueryWrapper<WorkShop> wrapper = new QueryWrapper<>();
