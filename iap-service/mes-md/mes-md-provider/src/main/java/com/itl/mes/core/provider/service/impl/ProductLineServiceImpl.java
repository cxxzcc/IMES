...
public class ProductLineServiceImpl extends ServiceImpl<ProductLineMapper, ProductLine> implements ProductLineService {
    ...
    public void deleteProductLineByProductLine( String productLine,Date modifyDate ) throws CommonException {
        ...
        final int count = shopOrderService.count(new QueryWrapper<ShopOrder>().lambda().eq(ShopOrder::getProductLineBo, productLineHandleBO.getBo()));
        if (count > 0) {
            throw new CommonException( "数据已被工单使用，无法执行删除操作", CommonExceptionDefinition.BASIC_EXCEPTION );
        }
        ...
        //查询产线是否被工位引用，已使用则不能删除
        final  int stationCount = stationService.count(new QueryWrapper<Station>().lambda().eq(Station::getProductLineBo,productLineHandleBO.getBo()));
        if(stationCount > 0){
            throw new CommonException( "数据已被工位使用，无法执行删除操作", CommonExceptionDefinition.BASIC_EXCEPTION );
        }
    ...
    public ProductLineVo getProductLineByProductLine( String productLine ) throws CommonException {
        ...
        if( !StrUtil.isBlank( productLineEntity.getWorkShopBo() ) ){
            Optional.ofNullable(workShopService.getById(productLineEntity.getWorkShopBo())).ifPresent(x -> productLineVo.setWorkShop(x.getWorkShop()));
        }
    ...
    public void export(HttpServletRequest request, HttpServletResponse response) {
              ...
              List<ProductLine> productLines = productLineMapper.selectList(queryWrapper);
              ExcelUtils.exportExcel(productLines, "产线列表", "产线列表", ProductLine.class, "产线列表.xls", true, response);


    }

    @Override
    public void saveByImport(List<ProductLineVo> list) {
         for (int i = 0; i < list.size(); i++){
             ProductLineVo productLineVo = list.get(i);
             if(productLineVo.getState() == null || productLineVo.getState() == ""){
                 productLineVo.setState("1");
             }
             String site  = UserUtils.getSite();
             ProductLineHandleBO productLineHandleBO = new ProductLineHandleBO( site, productLineVo.getProductLine());
             insertProductLine(site,UserUtils.getUserName(),productLineHandleBO.getBo(),productLineVo);
