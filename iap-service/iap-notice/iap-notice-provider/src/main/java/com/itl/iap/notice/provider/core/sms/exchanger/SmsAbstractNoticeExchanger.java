...
public class SmsAbstractNoticeExchanger extends AbstractNoticeExchanger {
    ...
    public boolean exchanger(Map<String, Object> map) {
        ...
        try {
            ...
            //记录
            saveSendMessage(msgType, msgPublicTemplate, title, sysNoticeContent, SendTypeEnum.SMS.getItem(),
                    SendStatusEnum.SEND_SUCCESS.getItem(), (String)map.get("businessId"), (String)map.get("referenceOrderNo"));
            return true;
        } catch (Exception e) {
            saveSendMessage(msgType, msgPublicTemplate, title, sysNoticeContent, SendTypeEnum.SMS.getItem(),
                    SendStatusEnum.SEND_FAIL.getItem(), (String)map.get("businessId"), (String)map.get("referenceOrderNo"));
            e.printStackTrace();
            throw new RuntimeException("map转对象失败");
        }
    }
