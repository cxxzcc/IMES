...
//        map.put("pageInfo",pageInfo);
     ...
     */
    @Override
    public IPage<Map<String, Object>> getMessageByUserName(String userId, Integer noticeType, Integer page, Integer pageNum, String searchText) {
        IPage<Map<String, Object>> ret = msgSendRecordMapper.findListByUsername(new Page(page, pageNum), userId, null, noticeType, searchText);
        ret.getRecords().stream().forEach(x -> {
            x.put("pcUrl",Optional.ofNullable(x.get("pcUrl")).orElse(""));
    ...
    @Override
    public Map<String, Object> getUnreadCountGroupByType(String userId) {
        List<Map<String, Object>> list = msgSendRecordMapper.getUnreadCountGroupByType(userId);
        if(CollUtil.isEmpty(list)) {
            return Collections.emptyMap();
        }
        return list.stream().collect(Collectors.toMap(e -> e.get("type").toString(), e -> e.get("count")));
    }
}
