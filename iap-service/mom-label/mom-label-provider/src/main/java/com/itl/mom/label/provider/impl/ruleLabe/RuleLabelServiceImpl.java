...
import com.itl.mom.label.provider.utils.GeneratorId;
...
public class RuleLabelServiceImpl extends ServiceImpl<RuleLabelMapper, RuleLabel> implements RuleLabelService {
    ...
    public void saveAndUpdate(RuleLabelSaveDto saveDto) throws CommonException {
        ...
        verify(saveDto.getBo(), saveDto.getRuleLabel(), saveDto.getElementType(), site);
    ...
    private void verify(String bo, String ruleLabel, String elementType, String site) throws CommonException {
        LambdaQueryWrapper<RuleLabel> query = new QueryWrapper<RuleLabel>().lambda()
                .eq(RuleLabel::getRuleLabel, ruleLabel)
                .eq(RuleLabel::getSite, site)
    ...
    @Override
    public List<String> generatorCode(RuleLabel ruleLabel, List<RuleLabelDetail> list, String elementBo, Integer number,Map<String,Object> paramsMap) throws CommonException {
    ...
    @Override
    public RuleLabelShowDto getByCode(String ruleLabel, String elementType) {
        return ruleLabelMapper.getByCode(ruleLabel,elementType);
    }
    ...
    @Override
    public Map<String, Object> getLabelParams(RuleLabel ruleLabel, List<RuleLabelDetail> list, String site, String elementBo) {
        if (list == null || list.size() == 0) {
            return Collections.emptyMap();
        }
        List<RuleLabelDetail> labelList = list.stream().filter(x -> StringUtils.isNotBlank(x.getTemplateArg())).collect(Collectors.toList());
        if (labelList == null || labelList.size() == 0) {
            return Collections.emptyMap();
        }
        Map<String, Object> params = this.getItemParams(list, site, elementBo, ruleLabel.getElementType());
        Map<String, Object> ret = new HashMap<>();
        labelList.forEach(x -> {
            ret.put(x.getTemplateArg(), params.get(x.getRuleVal()));
        });
        return ret;
    }
